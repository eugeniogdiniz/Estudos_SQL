/*1) Crie um database (nome: Consultorio ), com arquivo de dados localizado em
C:\BD\MDF\consultorio_dados.MDF e arquivo de log localizado em C:\BD\LDF\consultorio_log.ldf*/

CREATE DATABASE consultorio
ON PRIMARY(NAME = 'consultorio_Data',
FILENAME = 'C:\BD\MDF\consultorio_dados.MDF',
SIZE = 25 MB, 
MAXSIZE = 40 MB, 
FILEGROWTH = 10 MB)
LOG ON (NAME = 'consultorio_log', 
FILENAME ='C:\BD\LDF\consultorio_log.ldf',
SIZE = 5 MB, 
FILEGROWTH = 10 %)

/*2) Crie dois Filegroups adicionais, arquivo(.NDF), denominados:
- FG_dados ( D:\BD\MDF\FG_dados_consultorio.ndf )
- FG_historico ( E:\BD\MDF\FG_historico_consultorio.ndf )*/

alter database consultorio add Filegroup FG_dados
GO
alter database consultorio
add file (name = 'FG_dados_Data', filename = 'D:\BD\MDF\FG_dados_consultorio.ndf', size = 5MB, filegrowth=10%)
to Filegroup FG_dados
GO

alter database consultorio add Filegroup FG_historico
GO
alter database consultorio
add file (name = 'FG_historico_Data', filename = 'E:\BD\MDF\FG_historico_consultorio.ndf', size = 5MB, filegrowth=10%)
to Filegroup FG_dados

/*3) Altere os códigos de criação das tabelas ( fornecido ), para refletir as seguintes especificações:
- Tabelas Médico, Sala ficarão na partição PRIMARY.
- Tabelas Paciente na partição FG_dados
Faça a carga dos dados ( inserts ) nas tabelas citadas.*/
...
/*- Crie uma função de particionamento para a tabela Consulta, segregando os dados anteriores à 2022 dos
demais.
- Crie um esquema de partição, utilizando a função criada acima, para segregar os dados entre duas partições:
“anteriores à 2022” na partição FG_historico e “posteriores” ( >= 2022) na partição FG_dados.
- Crie a tabela Consulta orientando-a a seguir as regras da partição recentemente criada.
- Inclua ( ou garanta que existam ) valores com datas “anteriores à 2020”, entre “2021 e 2022” e atuais “>=2022 “  */

CREATE partition FUNCTION [PF_HISTORICO](datetime) AS range LEFT
FOR VALUES ( '2022-01-01 00:00:00.000')

CREATE partition scheme [PS_HISTORICO]
AS partition pf_historico TO ( [FG_dados], [PRIMARY])
...
CREATE TABLE Consulta (
	...
	, CONSTRAINT PK_Consulta PRIMARY KEY ( ID, DataHora ) ON PS_HISTORICO ( DataHora )
...
/*- Suponha que você tenha “errado a mão” e criado uma partição muito grande com dados anteriores à 2022 até
hoje como partição “historica”, então:
a) Faça um SPLIT da partição “historica” em 2 grupos, de “2021 a 2022” e ”anteriores a 2021”.*/

ALTER partition scheme pf_historico next used FG_dados;
ALTER partition function pf_historico() split range ('2021-01-01 00:00:00.000')

/*b) Faça um MERGE da partição de “2021 a 2022” com a partição atual ( que já continha dados >= 2022 ).
A situação final deve apresentar duas partições, histórica ( < 2021 ) e atual ( 2021+ ).*/

ALTER partition function pf_historico() merge range ('2019-01-01 00:00:00.000');